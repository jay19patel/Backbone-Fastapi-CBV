...
class RateLimiter:
    ...
    async def _get_redis(self, request: Request) -> redis.Redis | None:
    ...
    async def check(self, redis_client: redis.Redis, key: str) -> tuple[bool, int]:
        ...
        result = await redis_client.evalsha(sha, 1, key, now_ms, window_ms, self.calls)
...
class RateLimit:
    ...
    async def __call__(self, request: Request, response: Response):
        ...
        if not redis_client:
            ...
            logger.warning(
                "Rate limiting is enabled but Cache/Redis service is not running. Passing request."
            )
            return True
