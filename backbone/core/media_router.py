...
class MediaView(GenericCustomApiView):
    ...
    @classmethod
    def as_router(cls, prefix: str = "/media", tags: list = None, **kwargs) -> APIRouter:
        if tags is None:
            tags = ["Media"]
    ...
    async def handle_upload(
        ...
        if url:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to download image from URL: {str(e)}"
                )
        ...
        try:
            ...
            if file_url:
                response_url = (
                    ...
                    else f"{request.base_url}{file_url.lstrip('/')}"
                )
