...
class Category(BackboneDocument):
    ...
    img: Thumbnail = None
    color: str | None = Field(default=None, description="CSS color associated with category")
    description: str | None = Field(default=None, description="Short description of the category")
...
class Product(BackboneDocument):
    ...
    price_value: float | None = Field(default=0.0, description="Numeric price for calculations")
    img: Thumbnail = Field(default=None, json_schema_extra={"upload": False})
    images: list[str | Link[Attachment]] | None = Field(
        default_factory=list,
        description="Additional gallery images",
        json_schema_extra={"upload": False},
    )
    tag: str | None = Field(
        default=None, description="Special tag: Handmade, New, Bestseller, etc."
    )
    category_id: str | None = Field(default=None, description="Reference to Category ID")
    stock: int = Field(default=10, description="Items available in stock")
    description: str | None = Field(default=None, description="Full product description")
    details: str | None = Field(default=None, description="Comma-separated specification details")
    ...
    def serialize_images(self, images: list[Any] | None):
...
class CartItem(BackboneDocument):
    cart_id: str | None = Field(default=None, description="Internal ID of the parent cart")
...
class Cart(BackboneDocument):
    ...
    session_id: str | None = Field(default=None, description="Anonymous session ID for guest carts")
    ...
    is_ordered: bool = Field(
        default=False, description="True if this cart has been converted to an order"
    )
    order_id: str | None = Field(default=None, description="ID of the resulting order")
    ...
    class Settings:
        ...
        indexes = [
            ...
            # One active cart per guest session (logged-in carts must omit session_id — see CartView.before_create)
            # MongoDB partial indexes do not allow $exists: false / $ne null — only index session_id here.
            IndexModel(
...
class OrderItem(BaseModel):
    ...
    cart_item_id: str | None = Field(default=None, description="Original CartItem document ID")

    # Immutable snapshots
    name: str = Field(description="Product name at time of order")
    price: float = Field(description="Unit price at time of order")
    image: str | None = Field(default=None, description="Product image URL at time of order")
    quantity: int = Field(default=1)
...
class Order(BackboneDocument):
    ...
    payment_status: str = Field(
        default="pending", description="pending | received | verified | failed"
    )
    payment_id: str | None = Field(
        default=None, description="UPI/transaction ID submitted by customer"
    )
    payment: Link["Payment"] | None = Field(default=None, description="Link to Payment document")

    # Source cart
    cart_id: str | None = Field(default=None, description="Source Cart ID")
...
class Payment(BackboneDocument):
    ...
    transaction_id: str | None = Field(
        default=None, description="Customer-submitted UPI/transaction ID"
    )
    screenshot_url: str | None = Field(default=None, description="Optional payment screenshot URL")
    ...
    submitted_at: datetime | None = Field(
        default=None, description="When customer submitted payment details"
    )
    received_at: datetime | None = Field(
        default=None, description="When admin marks payment as received"
    )
    confirmed_at: datetime | None = Field(
        default=None, description="When admin confirms/verifies payment"
    )

    gateway_response: dict | None = Field(
        default=None, description="Raw gateway response if applicable"
    )
